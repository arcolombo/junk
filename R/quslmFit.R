...
#' @param object    expression set type
#' @param design    model.matrix built from formula
#' @param ndups     number of times each distinct probe is on array
#' @param spacing   the spacing between the next dupe, 1 for consecutive
#' @param block      vector or factor specifying a blocking variable on the arrays. Has length equal to the number of arrays. Must be ‘NULL’ if ‘ndups>2’.
#' @param correlation inter-duplicate of inter-technical replicate correlation
#' @param weights non-negative observation weights.  Can be a numeric matrix of individual weights, of same size as the object expression  matrix, or a numeric vector of array weights with length equal to ‘ncol’ of the expression matrix, or a numeric vector of gene weights with length equal to ‘nrow’ of the expression  matrix.
#' @param method fitting method; ‘"ls"’ for least squares or ‘"robust"’ for robust regression
#' @param ... other optional arguments for overloading lm.series, gls.series, or mrlm
quslmFit <- function(object,design=NULL,ndups=1,spacing=1,block=NULL,correlation,weights=NULL,method="ls",...)
