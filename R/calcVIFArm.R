...
#' @param labels labels for the columns of the eset, used in contrasting
#' @import limma 
#' @export 
...
#       return(mean(vif.grp,na.rm=T))
     ...
     # covar.mat = covar.mat / (ncol(eset) - length(grps))
      ...
      if(!is.null(geneResults$sd.alpha)){
       ...
       t<-calcVIFarm(names(geneResults$mean),geneResults$pathways[[i]],rownames(eset),eset,unique(labels), geneResults$sd.alpha) #dispatches for each i in geneResults$pathways list
